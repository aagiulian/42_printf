...
/*   Updated: 2017/02/08 17:31:07 by agiulian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void	ft_bint_conversion(t_flags *flags)
{
	if (!flags->h && !flags->l && !flags->j && !flags->j && !flags->z)
		flags->raw = ft_uitoabase(va_arg(*(flags->ap), unsigned int), 2);
	else if (flags->h == 2)
		...
		flags->raw = ft_uitoabase((size_t)va_arg(*(flags->ap), size_t), 2);
}

void		ft_bint_arg(t_flags *flags)
{
	ft_length_priority(flags);
	ft_bint_conversion(flags);
